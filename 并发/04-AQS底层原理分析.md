...
java.util.concurrent是在并发编程中比较常用的工具类，里面抱恨很多用来在并发场景中使用的组件。比如线程池、阻塞队列、计时器、同步器、并发集合等等。并发包的作者是大名鼎鼎的Doug Lea。我们在接下来的课程中，会去剖析一些经典的比较常用的组件的设计思想。
...
Lock本质上是一个接口，它定义了释放锁和获得锁的抽象方法，定义成接口就意味着它定义了锁的一个标准规范，也同时意味着锁的不同实现。实现Lock接口的类有很多，一下为几个常见的锁实现：
...
**设置head节点不需要用CAS，原因是设置head的节点是有获得锁的线程来完成的，而同步锁只能由一个线程获得，所以不需要CAS保证，只需要把head节点设置为源首节点的后继节点，并且断开源head节点的next引用即可。**
...
1. 非公平锁和公平锁最大的区别在于，在非公平锁中抢占锁的逻辑是，不管有没有线程拍段，我先上来CAS去抢占一下（加塞儿）。
2. CAS成功，就表示成功获得了锁
...
**PROPAGATE：**共享模式下，PROPAGATE状态的线程处于课运行状态。
...
如果是这个时候ThreadA释放锁了，那么我们来看锁释放后会缠身什么效果。
