...
**所谓自旋，就是指当有另外一个线程来竞争锁时，这个线程会在原地循环等待，而不是把该线程给阻塞，直到那个获得锁的线程释放锁之后，这个线程就可以马上获得锁了。**注意，锁在原地循环的时候，是会消耗CPU的，就相当于在执行一个啥也没有的for循环。

所以，轻量级锁适用于那些同步代码块执行很快的场景，这样线程原地等待很短的时间就能获得锁了。
...
假如有这样一个同步的代码块，存在Thread#1、Thread#2等多个线程：
